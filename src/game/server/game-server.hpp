...
private:
  std::vector<u8> serialize(const ServerMessage &message) override {
    ...
    std::visit(Overload(
                   [&](AckMessage message) {
                     packer.pack(ServerMessageType::ACK_MESSAGE);
                     packer.pack(message);
                   },
                   [&](IdMessage message) {
                     packer.pack(ServerMessageType::ID_MESSAGE);
                     packer.pack(message);
                   }),
               message);
  ...
  ClientMessage deserialize(const char *data, st length) override {
    ...
    switch (type) {
    case ClientMessageType::PLAY_MESSAGE:
      return payload.as<PlayMessage>();

    case ClientMessageType::MOVE_MESSAGE:
